...
class Solution {
    ...
    private static Result recursive(int[] alpha, int[] beta, int step){
        ...
        if(isOpponentWinner)
            return new Result(false, loseSteps.stream().max(Comparator.comparingInt(o->o)).get());
        return new Result(true, winSteps.stream().min(Comparator.comparingInt(o->o)).get());
